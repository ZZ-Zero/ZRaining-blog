(function($) {
  ...
  var Sidebar = function() {
    ...
    this.$button = $('#sidebar li.sidebar-button');
  ...
  Sidebar.prototype = {
    ...
    highLightLink: function() {
      var self = this;
      var path = location.pathname.replace(/(\s|\/)/g, '');
      self.$button.map(function(n, dom) {
        var href = $(dom).find('a').attr('href').replace(/(\s|\/)/g, '');
        console.log(href + ' : ' + path, (href === path));
        if (href === path) {
          $(dom).addClass('action');
        }
      });
